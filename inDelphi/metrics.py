...
def accuracy_type_agreement(x,y, top = 3):
	...
	x = np.asarray(x, dtype=np.float) #.argsort()[-top:][::-1]
	y = np.asarray(y, dtype=np.float) #.argsort()[-top:][::-1]
...
def stats_frequency_mutations(df, col, reference, mutcol = 'type'):
	...
	for index, row in df.iterrows():
		mut = row[mutcol].upper()
		if mut in reference:
			reference[mut] += float(row[col])
	return reference
